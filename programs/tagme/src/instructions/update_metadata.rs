...
use crate::{error::TagMeError, state::{PRODUCT_ACC_SEED, ProductRegistry, ProductStatus}};
...
pub fn handler(ctx: Context<UpdateMetadata>, new_hash: [u8; 32]) -> Result<()>{
    ...
    require!(authority.key() == product.author_pubkey, TagMeError::Unauthorized);
    require!(product.status == ProductStatus::Active, TagMeError::ProductRevoked);
    require!(product.metadata_hash != new_hash, TagMeError::InvalidMetadata);
